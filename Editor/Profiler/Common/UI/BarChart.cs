...
namespace Unity.NetCode.Editor
{
    class BarChart : VisualElement
    {
        ...
        internal void ShowTotalDataLabels(bool show)
        {
            m_TotalBytesLabel.style.display = show ? DisplayStyle.Flex : DisplayStyle.None;
            m_TotalPacketsLabel.style.display = show ? DisplayStyle.Flex : DisplayStyle.None;
        }
        ...
        internal void Update(NetcodeFrameData frameData)
        {
            ...
            if (m_NetworkRole == NetworkRole.Server)
            {
                ...
                totalPackets = frameData.totalSnapshotCountSentByServer;
            }
            else
            {
                totalBits = frameData.totalSizeReceivedByClientInBits;
                totalPackets = frameData.totalSnapshotCountReceivedByClient;
            }
