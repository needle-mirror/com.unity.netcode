...
NetDbg.prototype.SnapshotWidth = 15;
NetDbg.prototype.SnapshotMargin = 5;
NetDbg.prototype.mtuBytes = 1400;
NetDbg.prototype.mtuBits = NetDbg.prototype.mtuBytes*8;
...
NetDbg.prototype.updateNames = function(nameList) {
    ...
    if (this.content[con] == undefined) {
        ...
        //console.log(`Found NetCode NetworkConnection index:${con} from nameList:${nameList}`);

        const autoExpand = con <= 1; // Always expand the server & first client.
        if (!autoExpand)
...
NetDbg.prototype.wsReceive = function(evt) {
    ...
    if (typeof(evt.data) == "string") {
        ...
        var maxPackets = Math.ceil(totalSizeBits / this.mtuBits);
...
NetDbg.prototype.updateDrag = function(evt) {
    ...
    if (this.dragStarted) {
        ...
        // No need to clamp the upper bound.
        //var maxOffset = this.maxOffset()[0];
...
NetDbg.prototype.select = function(evt) {
    ...
    let offsetX = this.currentOffset();
    offset += offsetX;
    this.selection = Math.floor((offset + this.SnapshotMargin*0.5) / this.SnapshotWidth);
    this.tryDrawSelection();

    // Pause scroll when selecting:
    this.offsetX = offsetX;
    document.getElementById("liveUpdate").checked = false;
}

NetDbg.prototype.tryDrawSelection = function(evt) {
    if(this.drawnSelection === this.selection) return;
    for (var con = 0; con < this.content.length; ++con) {
        var content = this.content[con];
        ...
        if (this.selection >= 0 && this.selection < content.frames.length) {
            ...
            var selectedFrame = content.frames[this.selection];
            if(selectedFrame === undefined) continue;
            this.drawnSelection = this.selection;
            for (var i = 0; i < selectedFrame.snapshot.length; ++i) {
                var type = selectedFrame.snapshot[i];
                if (type.count == 0)
                    continue;
                ...
                totalSizeBits += type.size;
            }
            if (selectedFrame.predictionError != undefined) {
                ...
                for (var err = 0; err < content.errors.length; ++err) {
                    if (content.enabledErrors[err]) {
                        ...
                        errorTd.textContent = selectedFrame.predictionError[err];
            ...
            for (var t = 0; t < selectedFrame.time.length; ++t) {
                avgCommandAge += selectedFrame.time[t].commandAge / selectedFrame.time.length;
                avgTimeScale += selectedFrame.time[t].scale / selectedFrame.time.length;
                avgInterpolation += selectedFrame.time[t].interpolation / selectedFrame.time.length;
                avgInterpolationScale += selectedFrame.time[t].interpolationScale / selectedFrame.time.length;
                avgRTT += selectedFrame.time[t].rtt / selectedFrame.time.length;
                avgJitter += selectedFrame.time[t].jitter / selectedFrame.time.length;
                avgSnapshotAgeMin += selectedFrame.time[t].snapshotAgeMin / selectedFrame.time.length;
                avgSnapshotAgeMax += selectedFrame.time[t].snapshotAgeMax / selectedFrame.time.length;
            }
            ...
            var tickText = "Server tick " + selectedFrame.serverTick;
            tickText += " (" + (this.selection>0?(selectedFrame.serverTick - content.frames[this.selection-1].serverTick):0) + ")";
            tickText += " Time scale " + avgTimeScale.toFixed(2);
            tick.appendChild(document.createTextNode(tickText));

            //var ageText = "Snapshot age " + selectedFrame.snapshotAge.toFixed(2);
            ...
            for (var i = 0; i < selectedFrame.snapshotTicks.length; ++i) {
                snapText += i>0?", ":"" + selectedFrame.snapshotTicks[i];
            }
            snapText += "] ";
            snapText += Math.round(totalSizeBits / 8) + " bytes (" + totalSizeBits + " bits)";
            snapshots.appendChild(document.createTextNode(snapText));

            var cmdText = "Command ticks [";
            for (var i = 0; i < selectedFrame.commandTicks.length; ++i) {
                cmdText += i>0?", ":"" + selectedFrame.commandTicks[i];
            }
            cmdText += "] " + selectedFrame.commandSize + " bytes";
            commands.appendChild(document.createTextNode(cmdText));
            if (selectedFrame.discardedPackets > 0) {
                discard.appendChild(document.createTextNode("Discarded " + selectedFrame.discardedPackets + " packets"))
            }
        }
    }
    this.invalidate();
...
NetDbg.prototype.currentOffset = function() {
    if (this.offsetX >= 0)
        return this.offsetX; // liveUpdate is off here.

    // liveUpdate is hard to parse when it's a continuous scroll, so instead we shift only when we hit the end again.
    let maxOffsetTuple = this.maxOffset();
    maxOffset = maxOffsetTuple[0];
    let maxContentWidth = maxOffsetTuple[1];
    if(maxOffset <= 0) {
        return maxOffset;
        this.smoothedMaxOffset = maxOffset;
    }
    const jumpPerc = .85; // Denotes what fraction of the graph's width to slide the bars over, when we hit the end.
    maxOffset += maxContentWidth;
    maxOffset -= (maxOffset % (maxContentWidth*jumpPerc)) + (maxContentWidth * (1-jumpPerc));
    if(this.smoothedMaxOffset === undefined) this.smoothedMaxOffset = maxOffset;
    this.smoothedMaxOffset += (maxOffset - this.smoothedMaxOffset) * .2;
    return this.smoothedMaxOffset;
}
NetDbg.prototype.maxOffset = function() {
    ...
    for (var con = 0; con < this.content.length; ++con) {
        ...
        if (ofs > maxOffset[0]) maxOffset[0] = ofs;
        if (this.content[con].container.offsetWidth > maxOffset[1]) maxOffset[1] = this.content[con].container.offsetWidth;
    }
    return maxOffset;
}
...
NetDbg.prototype.present = function() {
    ...
    for (var con = 0; con < this.content.length; ++con) {
        ...
        const fullPacketIsPercOfHeight = 0.8;
        var bitScale = ((snapshotHeight * fullPacketIsPercOfHeight) / content.maxPackets) / this.mtuBits;

        // Each world's black graph bg:
        content.ctx.fillStyle = "black";
        content.ctx.fillRect(0,0,content.canvas.width, content.canvas.height);

        // Horizontal packet lines:
        {
            // Every Nth bytes is highlighted with a thin grey bar:
            let keyWidth = 30;
            let keyHeight = 15;
            const everyNBytes = 100;
            // Ensure each step is visible.
            const keysToShow = (bitScale * 8 * everyNBytes < keyHeight * 1.8) ? 4 : 1;
            if(content.maxPackets <= 5) {
                for (var j = 1; j <= 100; ++j) {
                    content.ctx.fillStyle = "#808080";
                    let bytesMarkerBarXOffset = bitScale * 8 * everyNBytes * j;
                    if(bytesMarkerBarXOffset >= snapshotHeight) break;
                    content.ctx.fillRect(0,snapshotHeight - bytesMarkerBarXOffset,content.canvas.width, 1);
                    // Key:
                    if(j % keysToShow == 0) {
                        content.ctx.fillRect(content.canvas.width - keyWidth, snapshotHeight - bytesMarkerBarXOffset - keyHeight*0.5, content.canvas.width, keyHeight);
                        content.ctx.fillStyle = "black";
                        content.ctx.fillText(`${everyNBytes * j}`, content.canvas.width - keyWidth + 5, snapshotHeight - bytesMarkerBarXOffset + 4, content.canvas.width, keyHeight);
                    }
                }
            }

            // Every 1 MTU is highlighted with a THICK bar:
            keyWidth = 40;
            keyHeight = 15;
            for (var i = 1; i <= content.maxPackets; ++i) {
                content.ctx.fillStyle = "white";
                let packetBarXOffset = this.mtuBits*bitScale*i;
                content.ctx.fillRect(0,snapshotHeight - packetBarXOffset,content.canvas.width, 3);
                // Key:
                content.ctx.fillRect(content.canvas.width - keyWidth, snapshotHeight - packetBarXOffset - keyHeight*0.5, content.canvas.width, keyHeight);
                content.ctx.fillStyle = "black";
                content.ctx.fillText(`${i} MTU`, content.canvas.width - keyWidth + 5, snapshotHeight - packetBarXOffset + 4, content.canvas.width, keyHeight);
            }
        ...
        // Pink selector highlight.
        if (this.selection >= 0) {
            content.ctx.fillStyle = "#fc0fc0";
            const selectorWidthMultiplier = 1.3 * this.SnapshotWidth;
        ...
        for (var i = 0; i < content.frames.length; ++i) {
            ...
            for (var t = 0; t < content.frames[i].snapshot.length; ++t) {
                ...
                content.ctx.fillRect(i*this.SnapshotWidth - currentOffset, snapshotHeight - bitScale * (totalSizeBits + content.frames[i].snapshot[t].size), this.SnapshotWidth-this.SnapshotMargin, bitScale * content.frames[i].snapshot[t].size);
                totalSizeBits += content.frames[i].snapshot[t].size;
            ...
            if (totalCount > 0) {
                ...
                if (uncompressedAlpha > 0.1) {
                    ...
                    content.ctx.strokeRect(i*this.SnapshotWidth - currentOffset, snapshotHeight-bitScale*totalSizeBits, this.SnapshotWidth-this.SnapshotMargin, bitScale*totalSizeBits);
                }
            }
            if (content.frames[i].commandSize > 0) {
                content.ctx.fillStyle = this.Colors[0];
                content.ctx.fillRect(i*this.SnapshotWidth - currentOffset, snapshotHeight+commandHeight - bitScale * content.frames[i].commandSize*8, this.SnapshotWidth-this.SnapshotMargin, bitScale * content.frames[i].commandSize*8);
            }
            ...
            // Vertical timeline lines, one for each snapshot tick.
            {
                ...
                var xpos = i*this.SnapshotWidth - currentOffset - (this.SnapshotMargin/2);
        ...
        content.ctx.fillStyle = "white";
        content.ctx.fillRect(0,snapshotHeight,content.canvas.width, 5);
        content.ctx.fillRect(0,snapshotHeight+commandHeight,content.canvas.width, 5);
