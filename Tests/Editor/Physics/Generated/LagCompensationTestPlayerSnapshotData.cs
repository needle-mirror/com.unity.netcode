...
public struct LagCompensationTestPlayerSnapshotData : ISnapshotData<LagCompensationTestPlayerSnapshotData>
{
    public uint tick;
    private int LagCompensationTestPlayerOwner;
    ...
    public int GetLagCompensationTestPlayerOwner(GhostDeserializerState deserializerState)
    {
        return (int)LagCompensationTestPlayerOwner;
    }
    public int GetLagCompensationTestPlayerOwner()
    {
        return (int)LagCompensationTestPlayerOwner;
    }
    public void SetLagCompensationTestPlayerOwner(int val, GhostSerializerState serializerState)
    {
        LagCompensationTestPlayerOwner = (int)val;
    }
    public void SetLagCompensationTestPlayerOwner(int val)
    {
        LagCompensationTestPlayerOwner = (int)val;
    }
    ...
    public void Serialize(int networkId, ref LagCompensationTestPlayerSnapshotData baseline, ref DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (LagCompensationTestPlayerOwner != baseline.LagCompensationTestPlayerOwner) ? 1u : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedIntDelta(LagCompensationTestPlayerOwner, baseline.LagCompensationTestPlayerOwner, compressionModel);
    ...
    public void Deserialize(uint tick, ref LagCompensationTestPlayerSnapshotData baseline, ref DataStreamReader reader,
        NetworkCompressionModel compressionModel)
    {
        this.tick = tick;
        changeMask0 = reader.ReadPackedUIntDelta(baseline.changeMask0, compressionModel);
        if ((changeMask0 & (1 << 0)) != 0)
            LagCompensationTestPlayerOwner = reader.ReadPackedIntDelta(baseline.LagCompensationTestPlayerOwner, compressionModel);
        else
            LagCompensationTestPlayerOwner = baseline.LagCompensationTestPlayerOwner;
        if ((changeMask0 & (1 << 1)) != 0)
        {
    ...
    public void Interpolate(ref LagCompensationTestPlayerSnapshotData target, float factor)
    {
