...
namespace Unity.NetCode.Tests
{
    public class RpcTests
    {
        ...
        [Test]
        [Ignore("Need significant package hardening to make guarantees about what happens when fuzzing packets!")]
        ...
        public void Rpc_IndividualRpcTooLarge_ThrowsAndLogError()
        {
            using (var testWorld = new NetCodeTestWorld())
            {
                ...
                unsafe
                {
                    var ptr = (int*)&serializedTooBigCommand.bytes;
                    for (int i = 0; i < (sizeof(SerializedTooBigCommand) / 4); i++) ptr[i] = i;
                }
