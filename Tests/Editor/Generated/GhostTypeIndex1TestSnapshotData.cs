...
public struct GhostTypeIndex1TestSnapshotData : ISnapshotData<GhostTypeIndex1TestSnapshotData>
{
    public uint tick;
    private int GhostTypeIndexValue;
    ...
    public int GetGhostTypeIndexValue(GhostDeserializerState deserializerState)
    {
        return (int)GhostTypeIndexValue;
    }
    public int GetGhostTypeIndexValue()
    {
        return (int)GhostTypeIndexValue;
    }
    ...
    public void SetGhostTypeIndexValue(int val)
    {
        GhostTypeIndexValue = (int)val;
    }
    ...
    public void Serialize(int networkId, ref GhostTypeIndex1TestSnapshotData baseline, ref DataStreamWriter writer, NetworkCompressionModel compressionModel)
    {
        changeMask0 = (GhostTypeIndexValue != baseline.GhostTypeIndexValue) ? 1u : 0;
        ...
        if ((changeMask0 & (1 << 0)) != 0)
            writer.WritePackedIntDelta(GhostTypeIndexValue, baseline.GhostTypeIndexValue, compressionModel);
    ...
    public void Deserialize(uint tick, ref GhostTypeIndex1TestSnapshotData baseline, ref DataStreamReader reader,
        NetworkCompressionModel compressionModel)
    {
        this.tick = tick;
        changeMask0 = reader.ReadPackedUIntDelta(baseline.changeMask0, compressionModel);
        if ((changeMask0 & (1 << 0)) != 0)
            GhostTypeIndexValue = reader.ReadPackedIntDelta(baseline.GhostTypeIndexValue, compressionModel);
        else
            GhostTypeIndexValue = baseline.GhostTypeIndexValue;
        if ((changeMask0 & (1 << 1)) != 0)
        {
    ...
    public void Interpolate(ref GhostTypeIndex1TestSnapshotData target, float factor)
    {
