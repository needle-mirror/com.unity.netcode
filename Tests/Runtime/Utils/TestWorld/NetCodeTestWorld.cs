...
namespace Unity.Netcode.Tests
{
    ...
    internal class NetCodeTestWorld : IDisposable, INetworkStreamDriverConstructor, IAsyncDisposable, IPrebuildSetup, IPostBuildCleanup
    {
        ...
        public NetCodeTestWorld(double initialElapsedTime = 42, bool keepExistingNetcodeInstance = false)
        {
            ...
            {
                if (m_OldGlobalConfig != null) m_OldGlobalConfig.m_IsGlobalConfig = false;
                m_GlobalConfigForTests.m_IsGlobalConfig = true;
            }
        ...
        public void Dispose()
        {
            ...
            if (m_OldGlobalConfig != null)
                m_OldGlobalConfig.m_IsGlobalConfig = true;
        ...
        {
            // OrdinalIgnoreCase: asmdefs are being renamed Unity.NetCode -> Unity.Netcode, tolerate both during the migration.
            return sys.Assembly.FullName.StartsWith("Unity.Netcode,", StringComparison.OrdinalIgnoreCase) ||
                ...
                sys.Assembly.FullName.StartsWith("Unity.Netcode.Editor,", StringComparison.OrdinalIgnoreCase) ||
                sys.Assembly.FullName.StartsWith("Unity.Netcode.Editor.Tests,", StringComparison.OrdinalIgnoreCase) ||
                sys.Assembly.FullName.StartsWith("Unity.Netcode.GhostAdapterRuntime.Tests,", StringComparison.OrdinalIgnoreCase) ||
                sys.Assembly.FullName.StartsWith("Unity.Netcode.Physics.Editor.Tests,", StringComparison.OrdinalIgnoreCase) ||
                sys.Assembly.FullName.StartsWith("Unity.Netcode.TestsUtils.Runtime.Tests,", StringComparison.OrdinalIgnoreCase) ||
        ...
        public void MigrateServerWorld(NetcodeWorld suppliedWorld = null)
        {
            ...
            NetcodeWorld newWorld = migrationSystem.LoadWorld(ticket, suppliedWorld);
        ...
        public void MigrateClientWorld(int index, NetcodeWorld suppliedWorld = null)
        {
        ...
        public virtual void Setup()
        {
            ...
            {
                if (netCodeConfig.m_IsGlobalConfig)
                {
                    netCodeConfig.m_IsGlobalConfig = false;
                    m_OldGlobalConfig = netCodeConfig;
                    break;
                }
            }
            NetcodeConfig.Global = m_TempBootstrapConfig;
            m_TempBootstrapConfig.m_IsGlobalConfig = true;
        }

        public virtual void Cleanup()
        {
            Object.DestroyImmediate(m_TempBootstrapConfig);
            if (m_OldGlobalConfig != null) m_OldGlobalConfig.m_IsGlobalConfig = true;
        }
