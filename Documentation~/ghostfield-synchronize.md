...
Use these properties to customize how components and buffers are serialized using `GhostFieldAttribute`. For more details, refer to the [`GhostFieldAttribute` API documentation](xref:Unity.Netcode.GhostFieldAttribute).

| Property | Default value | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
|---|---|--------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| [`Quantization`](xref:Unity.Netcode.GhostFieldAttribute.Quantization) | Disabled by default on float and unavailable on integers. | Use the `Quantization` property to set [quantization](optimization/compression.md#quantization) for floating point numbers (and other supported types, refer to [ghost type templates](ghost-types-templates.md)) to limit the precision of data. The floating point number is multiplied by the quantization value and converted to an integer to save bandwidth.                                                                                                                                                                                                                                                             |
| [`Composite`](xref:Unity.Netcode.GhostFieldAttribute.Composite) | Disabled by default. | Use the `Composite` property to control how [delta compression](optimization/compression.md#delta-compression) computes the change field's bitmask for non-primitive fields (such as structs). When set to `true`, delta compression templating generates only one bit to indicate whether the entire struct contains any changes. If `Composite` is false, each field has its own change-bit. Use `Composite=true` if all fields are typically modified together (for example, `GUID`).                                                                                                                                       |
| [`SendData`](xref:Unity.Netcode.GhostFieldAttribute.SendData) | Enabled by default. | Use the `SendData` property to instruct code generation not to include a field in the serialization data. This is particularly useful for non-primitive members (like structs) which have all fields serialized by default.                                                                                                                                                                                                                                                                                                                                                                                                    |
| [`Smoothing`](xref:Unity.Netcode.GhostFieldAttribute.Smoothing)| Default setting is [`Clamp`](xref:Unity.Netcode.SmoothingAction.Clamp). | Use the `Smoothing` property to control how a field is updated when the ghost is in `GhostMode.Interpolated`. Options are `Clamp` (every time a snapshot is received, clamp the client value to the latest snapshot value), `Interpolate` (interpolate the field between the last two snapshot values every frame; if no data is available for the next tick, clamp to the latest value), and `InterpolateAndExtrapolate` (interpolate the field between the last two snapshot values every frame; if no data is available for the next tick, the next value is linearly extrapolated using the previous two snapshot values). |
| [`MaxSmoothingDistance`](xref:Unity.Netcode.GhostFieldAttribute.MaxSmoothingDistance) |  | Use the `MaxSmoothingDistance` property to disable interpolation when the values change more than the specified limit between two snapshots. This is useful for dealing with teleportation, for example.                                                                                                                                                                                                                                                                                                                                                                                                                       |
| [`SubType`](xref:Unity.Netcode.GhostFieldAttribute.SubType) |  | Use the `SubType` property to specify a custom serializer for a field using the [`GhostFieldSubType`](xref:Unity.Netcode.GhostFieldSubType) API.                                                                                                                                                                                                                                                                                                                                                                                                     |

> [!NOTE]
> Ghosts that are marked as **both** static-optimized **and** interpolated will **never** extrapolate,
...
You can use the [`SendData` property](xref:Unity.Netcode.GhostFieldAttribute.SendData) to skip serialization and replication of a field, which means that:
...
Dynamic buffer fields don't support [`SmoothingAction`](xref:Unity.Netcode.SmoothingAction) so the `GhostFieldAttribute.Smoothing` and `GhostFieldAttribute.MaxSmoothingDistance` properties are ignored on buffers.
...
When using automated input synchronization with [`IInputComponentData`](xref:Unity.Netcode.IInputComponentData):

[!code-cs[blobs](../Tests/Editor/DocCodeSamples/ghostfield-synchronize.cs#IInputComponentData)]

[`ICommandData`](xref:Unity.Netcode.ICommandData) is a subclass of [`IBufferElementData`](xref:Unity.Entities.IBufferElementData) and can be serialized for replication from the server to clients. As such, the same rules as for [buffers](#dynamic-buffer-serialization) apply: if the command buffer is to be serialized, then all fields must be annotated.
...
- Provide a custom serialization template for a type and target it using the [`SubType` property](xref:Unity.Netcode.GhostFieldAttribute.SubType) of `GhostFieldAttribute`.
...
- [`GhostFieldAttribute` API documentation](xref:Unity.Netcode.GhostFieldAttribute)
- [`Unity.Entities.IComponentData` API documentation](xref:Unity.Entities.IComponentData)
- [`Unity.Entities.IBufferElementData` API documentation](xref:Unity.Entities.IBufferElementData)
